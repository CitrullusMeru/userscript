// ==UserScript==
// @name         Countdown Timer for the Random Encounter Event on E-H (Safari Fixed)
...
(function() {
    ...
    var set_cookie = function(k, v, t) {
        ...
        var domainMatch = /[^\.]+\.[^\.]+$/.exec(loc.hostname);
        var domain = domainMatch ? '; domain=.' + domainMatch[0] : '';
        doc.cookie = k + '=' + encodeURIComponent(v) + expires + domain + '; path=/';
    };
    ...
    if(!get_cookie('event')) { 
        console.log('The "event" cookie does not exist or is invalid.'); 
        return; // 用 return 代替 throw
    }
                ...
                try {
                    if(doc.getElementById('eventpane').getElementsByTagName('div')[1].getElementsByTagName('a')[0].textContent != 'HentaiVerse') {
                        if(!doc.getElementById('re_snd')) {
                            ...
                            audio.volume = 1;
                            audio.loop = true;
                            // 加上 catch 捕捉 Safari 封鎖自動播放的錯誤，避免腳本死掉
                            audio.play().catch(function(err) { console.log("Safari blocked autoplay audio: ", err); });
